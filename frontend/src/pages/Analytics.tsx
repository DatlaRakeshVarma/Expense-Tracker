...
const Analytics: React.FC = () => {
  ...
  const filterExpensesByTimeframe = (expenses: any[], timeframe: TimeframeType) => {
  ...
  const getChartData = () => {
    if (!filteredExpenses.length) return { categoryData: [], monthlyData: [], totalSpending: 0 };
  ...
  const { categoryData, monthlyData, totalSpending } = getChartData();
